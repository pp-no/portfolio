...
export default function LogosSection() {
  const logos = new Array(12).fill(0).map((_, i) => `https://placehold.co/120x40?text=Logo+${i + 1}`);
  return (
    <section id="logos" className="py-16">
      <div className="mx-auto max-w-6xl px-4">
        ...
        <div className="mt-8 grid grid-cols-2 items-center gap-6 opacity-80 sm:grid-cols-4 lg:grid-cols-6">
          {logos.map((src) => (
            <div key={src} className="flex items-center justify-center rounded border bg-white px-4 py-3 shadow-sm">
