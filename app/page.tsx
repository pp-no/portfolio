...
const profileParagraphs = [
	'バックエンドではPHP、ASP.NET C#、C++などを用いた開発経験があり、既存システムの運用改善から新規実装まで幅広く対応してきました。',
	'PHPの実務ではMVCを前提としたスクラッチ開発、SQLを用いたデータ処理、保守を見据えたサーバーサイド実装に継続して取り組んできました。',
	'フロントエンドではjQueryに加えてReact、Next.js、TypeScriptを用いた実装経験があり、見た目だけでなく、再利用しやすい構成や変更に強いUIを意識して設計・実装しています。',
	'現在はフロントエンドを主軸にしつつ、APIやデータ設計まで含めて全体を見ながら進めることが多く、必要に応じてフルスタックに動けることを強みにしています。また、AIはワークフロー設計、ソースレビュー、実装設計書の作成支援に活用し、チーム開発で属人化しにくい進め方を支えるAIペアプロとして取り入れています。',
	'新しい技術は流行として追うのではなく、開発速度、保守性、チームで扱いやすいかという観点で見極めながら取り入れ、継続的に価値を出せる実装を心がけています。',
];
